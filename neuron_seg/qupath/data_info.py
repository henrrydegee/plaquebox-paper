...
with open('cell_wo_residue.csv') as csvfile:
    ...
    temp = np.zeros((1001,1001))
    x, y = np.where(temp==0)
    dist_matrix = distance_matrix(np.dstack((x,y))[0], [[1001, 1001]])
        ...
        for row in readCSV:
            if len(row) > 0:
                ...
                if x != 'NaN' and y != 'NaN':
                    ...
                    if (int(x)>=500) and (int(x)<height-500) and (int(y)>=500) and (int(y) < width-500):
                        start_epoach = time.time()
                        # Create a box shape 301*301, center is (150,150)
                        box_coords = datapoints[int(x)-500:int(x)+501, int(y)-500:int(y)+501]
    ...
    with open("feature_white_1000.csv","w", newline="") as f:
        writer = csv.writer(f)
