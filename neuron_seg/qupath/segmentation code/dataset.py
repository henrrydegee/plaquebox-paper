...
with open('cell_wo_residue.csv') as csvfile:
    ...
    i = 0
    with open('NA3777-02_AB.csv') as csvfile2:
        ...
        for row in readCSV:
            if len(row) == 2:
                ...
                if (x+400 < (int(max(x_list))+1)) and (x-400 > 0) and (y+400 < (int(max(y_list))+1)) and (y-400 > 0):
                    box_coords = datapoints[x-400:x+400, y-400:y+400]
                #print(points)
                im = Image.fromarray(box_coords)
                img_name = "./boundary/" + str(i) + "_boundary.png"
                im.save(img_name)
